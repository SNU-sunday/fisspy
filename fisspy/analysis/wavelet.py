...
Wavelet
 ...
 The first Python 2 version of the code is written by Evgeniya Predybaylo in 2014
 Python 3 version code is written by Juhyeong Kang in sep 2016
...
def wavelet(Y, dt, pad=False, dj=False, s0=False, J1=False, mother=False, param=False):
	n1 = len(Y)
	...
	j = np.arange(J1+1,dtype=float)
	scale = s0*2.**(j*dj)
	wave = np.empty((J1+1, n), dtype=complex)  # define the wavelet array
